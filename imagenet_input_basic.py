...
class ImageNetTFExampleInput(object):
  ...
  def input_fn(self, params):
    ...
    dataset = dataset.map(self.dataset_parser).batch(batch_size=batch_size, drop_remainder=True)
  ...
  def make_source_dataset(self, index, num_hosts):
    ...
    if not self.dataset_split:
      ...
      if self.dataset_split not in ['train', 'validation', 'test']:
        raise ValueError(
            "If provided, dataset_split must be 'train' or 'validation' or 'test', was %s"
            % self.dataset_split)
    ...
    dataset = dataset.apply(tf.data.experimental.filter_for_shard(num_hosts, index))
      
    def fetch_dataset(filename):
      buffer_size = 8 * 1024 * 1024 # 8 MiB per file
      dataset = tf.data.TFRecordDataset(filenames = filename, buffer_size=buffer_size)
      return dataset
      
    dataset = dataset.apply(
        tf.data.experimental.parallel_interleave(map_func=fetch_dataset, cycle_length=self.num_parallel_calls, sloppy=True))
        
    if self.cache:
      dataset = dataset.cache().apply(
          tf.data.experimental.shuffle_and_repeat(buffer_size=self.batch_size*16))
    else:
