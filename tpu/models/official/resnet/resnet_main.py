...
from tensorflow.contrib import summary, slim
...
flags.DEFINE_integer(
    'predict_batch_size', default=1024 , help='')
    
flags.DEFINE_string(
    'tpu_profile_duration', default='2000' , help='')
    
flags.DEFINE_integer(
    'num_test_images', default=1024, help='')
...
def init_weights(path):
    ...
    if model_path:
        ...
        variables_to_restore = slim.get_variables_to_restore(exclude=["dense"])
...
def resnet_model_fn(features, labels, mode, params):
  ...
  var_list = [v for v in tf.trainable_variables() if v.name.find('dense/') == 0]
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    # only last layer vars must be trained
    with tf.control_dependencies(update_ops):
  ...
  if params['resnet_checkpoint'] and mode == tf.estimator.ModeKeys.TRAIN:
    def scaffold_fn():
      return tf.train.Scaffold(init_fn=init_weights(params['resnet_checkpoint']))
  else:
    scaffold_fn = None
...
def main(unused_argv):
  ...
  params['resnet_checkpoint'] = FLAGS.resnet_checkpoint
  ...
  if FLAGS.mode == 'predict':
    eval_results = resnet_classifier.evaluate(
            input_fn = imagenet_test(True, params, use_bfloat16).input_fn,
            steps = FLAGS.num_test_images // FLAGS.predict_batch_size)
    tf.logging.info('Eval results test %s', eval_results)
    
    results = resnet_classifier.predict(input_fn = imagenet_test(False, params, use_bfloat16).input_fn)
  ...
  if FLAGS.mode == 'eval':
    ...
    for ckpt in evaluation.checkpoints_iterator(
      ...
      try:
        ...
        if current_step > 0:
          current_step = current_step - 112603
    ...
    if FLAGS.mode == 'train':
      ...
      if FLAGS.profile_every_n_steps > 0:
        hooks.append(
            tpu_profiler_hook.TPUProfilerHook(
                ...
                tpu_profiler_command=["/usr/local/bin/capture_tpu_profile", "--duration_ms=" + FLAGS.tpu_profile_duration],
      ...
      while current_step < params['train_steps']:
        ...
        resnet_classifier.train(
            input_fn=imagenet_train.input_fn, steps=next_checkpoint, hooks=hooks)
