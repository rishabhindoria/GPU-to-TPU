...
def _process_dataset(filenames, synsets, labels, output_directory, prefix,
  ...
  chunksize = (len(filenames) + num_shards -1 ) / num_shards
  coder = ImageCoder()
...
def convert_to_tf_records(raw_data_dir):
  ...
  # Create test data
  tf.logging.info('Processing the test data.')
  test_records = _process_dataset(
      test_files, test_synsets, labels,
      FLAGS.local_scratch_dir,
...
def upload_to_gcs(training_records, validation_records, test_records):
  ...
  # Upload test dataset
  tf.logging.info('Uploading the test data.')
...
def main(argv):  # pylint: disable=unused-argument
  ...
  training_records, validation_records, test_records = convert_to_tf_records(raw_data_dir)

  # Upload to GCS
  if FLAGS.gcs_upload:
    upload_to_gcs(training_records, validation_records, test_records)
